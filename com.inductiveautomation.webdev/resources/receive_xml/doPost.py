def doPost(request, session):
	...
	# Define logger
	logger = system.util.getLogger('receive ERP_Integration')
...
# Get the incoming payload
	...
	try:
	    ...
	    if e1afkol is not None:
	    		...
	    		idoctype= edi_dc40.find('IDOCTYP').text
	    elif edi_dc40 is not None:
	    		idoctype= edi_dc40.find('IDOCTYP').text
	    		identifier= edi_dc40.find('DOCNUM').text		
	    else:
	    ...
	    params ={'messageIdentifier':identifier,'docType':docType,'status':'NEW','message':'Success: Data received from SAP','step':'NEW','payload':xml_data}
